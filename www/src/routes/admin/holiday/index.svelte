<script context="module">
    ...
    export async function preload({ params, query }, { user, token }) {
        ...
        let data;
        try {
            const response = await Api.list({ 'per-page': perPage, page, sort, ...filter, expand: 'country' }, token);
            if (response.success) {
                data = response.data;
            } else {
...
<script>
    ...
    let datatableAttributes = [
        ...
        { 
            ...
            value: model => model.country ? model.country.name : '-',
        },
        { 
            attribute: 'status',
            label: 'Status', 
