<script context="module">
    ...
    export async function preload({ query }, { token }) {
        ...
        let data;
        try {
            const response = await testIssueApi.assigned({ 
                'per-page': perPage, 
                page, 
                sort, 
                expand: 'group',
...
<script>
    ...
    const breadcrumb = [
        { label: 'Home', href: '/' },
        { label: 'My Issues', href: '/issue/assigned' },
    ];
    
    async function redirect() {
        let urlQuery = utils.urlQuery(query);
        await goto(`/issue/assigned?${urlQuery}`);
    }
    ...
    let datatableAttributes = [
        ...
        { 
            ...
            value: row => row.group ? `${row.group.name} (${row.group.test_status})` : '-'
        },
        { 
            attribute: 'name',
            label: 'Name', 
...
<PageTitle title="My Issues" {breadcrumb}>
</PageTitle>
